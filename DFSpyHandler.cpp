...
void DFSpyHandler::performBitlevelChecks( const std::map< daq::ftk::SourceIDSpyBuffer, std::shared_ptr<daq::ftk::SpyBuffer> >& ) {
    // Not yet implemented.
}

void DFSpyHandler::performBitLevelChecksTESTER(std::string dumpdir, bool parseDirectory=true, bool makeFrags = true, bool doParsing=true) {
    if(parseDirectory) {
 ...
 */
std::map<int, std::tuple<int, int> > DFSpyHandler::mapROBIDtoBoardAndLane(std::string multiboardfile) {
 ...
 */
std::map<std::tuple<int, int>, std::tuple<int, int> > DFSpyHandler::mapTowerPlaneToBoardLane(std::string sysFile) {
    // Declare maps:
 ...
 */
std::map<std::tuple<int, int>, std::set<int> > DFSpyHandler::mapBoardAndLaneToModIDsInspy(std::string moduleListFile) {
    ...
    if(myfile.is_open()) {
        while(std::getline(myfile, line) ) {
            ...
            std::tuple<int, int> boardAndLane = ROBIDtoBoardAndLane[robID];
 ...
 */
std::map<std::tuple<int, int>, std::set<int> > DFSpyHandler::mapBoardAndLaneModIDsOutspy(std::string moduleListFile) {
 ...
 */
std::map<int, std::set< std::tuple<int, int> > > DFSpyHandler::mapModID2BoardAndLaneOutspy() {
...
std::string DFSpyHandler::get2TupleString(std::tuple<int, int> t) {
    std::string tuple2 = " ";
    tuple2 = "( "  + std::to_string(std::get<0>(t)) + ", " + std::to_string(std::get<1>(t)) + " )";
    return tuple2;
}
